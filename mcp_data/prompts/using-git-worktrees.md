...
| 错误 | 修正 |
|------|------|
| 与 harness 对抗 | 步骤 0 检测现有隔离。步骤 1a 让位给原生工具 |
| 跳过检测 | 创建任何东西之前都先跑步骤 0 |
| 跳过忽略验证 | 创建项目本地 worktree 前始终使用 `git check-ignore` |
| 假设目录位置 | 遵循优先级：现有目录 > 全局历史路径 > instructions 文件 > 默认 |
| 带着失败的测试继续 | 报告失败，获得明确许可后再继续 |
...
- 在已有原生 worktree 工具（如 `EnterWorktree`）的情况下还用 `git worktree add`
